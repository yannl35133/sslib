...
from .constants import (
    REGULAR_DUNGEONS as POTENTIALLY_REQUIRED_DUNGEONS,
    SILENT_REALMS,
    SILENT_REALM_CHECKS,
)
...
class Hints:
    def __init__(self, stonehint_definitions, options: Options, rng, logic: Logic):
        self.stonehint_definitions = stonehint_definitions
        self.logic = logic
    ...
    def do_normal_hints(self):
        ...
        location_hints_left = self.options["location-hints"]
        location_hints = []
        ...
        # create sots hints
        sots_hints_count = self.options["sots-hints"]
        sots_hints = []
        if (
            sots_hints_count > 0
        ):  # avoid doing the additional sots calculations if it isn't necessary
            sots_locations = {}
            for sotsloc, item in self.logic.get_sots_locations().items():
                if self.options["small-key-mode"] not in [
                    "Anywhere",
            ...
            self.rng.shuffle(sots_zones)
            for i in range(sots_hints_count):
                loc = self.rng.choice(sots_locations[sots_zones[i]])
        ...
        # create barren hints
        barren_hints_count = self.options["barren-hints"]
        barren_hints = []
        if barren_hints_count > 0:
            ...
            for zone in region_barren:
                ...
                if self.options["empty-unrequired-dungeons"]:
                    ...
                    if self.options["skip-skykeep"] and zone == "Sky Keep":
                ...
                if zone == "Sky Keep":
                    ...
                    if self.options["map-mode"] not in [
                        "Removed, Anywhere"
                    ] or self.options["small-key-mode"] not in ["Anywhere"]:
                        continue
                barren_zones.append(zone)
            self.rng.shuffle(barren_zones)
        ...
        item_hints = []
        self.rng.shuffle(hintable_items)
        for i in range(self.options["item-hints"]):
        ...
        while hints_left > 0 and all_locations_without_hint:
            ...
            location_to_hint = self.rng.choice(all_locations_without_hint)
    ...
    def do_bingo_hints(self):
        ...
        if self.options["shop-mode"] == "Randomized":
    ...
    def _place_hints_for_locations(
        ...
        for gossipstone_name in [
            ...
            if len(unhinted_locations) == 0:
                ...
                continue
            loc_to_hint = self.rng.choice(unhinted_locations)
            unhinted_locations.remove(loc_to_hint)
            second_loc_to_hint = self.rng.choice(unhinted_locations)
            unhinted_locations.remove(second_loc_to_hint)
            hint_to_location[gossipstone_name] = [loc_to_hint, second_loc_to_hint]
